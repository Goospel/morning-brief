...
- [x] **Step 3: 설계 문서 정정 3건 반영**

`docs/superpowers/specs/2026-08-23-frontend-design.md`에서:
- ~~`granite.config.ts` → `apps-in-toss.config.ts` (4절·8절)~~ — **해당 없음**. 레포 전체를 검색해도 설계 문서에는 `granite.config.ts`가 한 번도 안 나온다(이 계획서와 changeLog에만 있다). 고칠 것이 없어 8절 T1에 `apps-in-toss.config.ts`가 올바른 이름이라고 적는 것으로 대신했다
- 스캐폴딩 명령에 `--tds --inline` 명시 (8절 T1) ✅
- 3절 T0 스파이크는 이미 「완료」로 반영돼 있다 — 변경 불필요
- (추가) 4절 미확인 ⓐ(`openURL`이 외부 브라우저인지) 해소 — SDK 3.0.2 타입 정의가 「기기의 기본 브라우저나 관련 앱」으로 명시. ⓑ(운영 정책 허용 여부)는 잔존
- (추가) 1절 unlink 항목에 fail-closed 구현 반영, 9절 미확인 표 #3·#4 갱신
- (추가) 2절에 `appLogin()` → `TossAuth.login()` 현행 API 주석
...
- [x] **Step 7: PR 생성** — [#7](https://github.com/Goospel/morning-brief/pull/7)
