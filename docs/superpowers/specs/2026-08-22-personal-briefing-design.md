...
- 문구는 **완전 고정 템플릿 1개**. 변수를 넣으면 사전 검수가 까다로워진다.
- **확정 문구(2026-08-24)** — 제목 `오늘의 브리핑` / 본문 `나에게 맞는 뉴스가 도착했어요`. 필드가 하나뿐이면 `오늘의 브리핑이 도착했어요`.
  - **시간 중립이 조건이다.** `push_hour` 가 0~23 이라 "좋은 아침" 류를 쓰면 22시로 맞춘 사용자에게 그대로 간다. 고정 문구 1개가 아무 시각에나 나가는 구조라 시간대 표현 자체를 못 쓴다.
  - 이모지·느낌표는 넣지 않는다 — 검수 리스크만 늘고 얻는 것이 없다.
  - 개인화를 문구에 담은 이유: 이 앱의 차별점이 그것뿐이라, "브리핑이 준비됐어요" 류는 어느 뉴스 앱이든 쓸 수 있는 말이 되어 열어볼 이유를 주지 못한다.
