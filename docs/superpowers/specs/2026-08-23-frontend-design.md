...
- ⚠️ **미확인**: 콜백 요청의 인증 방식(서명 헤더인지, 토스 측 mTLS인지, 페이로드 스키마). 콘솔에서 콜백 URL을 등록하는 시점에 문서·실페이로드로 확정한다. 확정 전 구현은 "페이로드의 userKey 존재 검증 + 전체 로깅"까지만 하고, 검증 수단이 확인되면 붙인다.
...
- **원문 보기** = `openURL(article.url)` (`@apps-in-toss/web-framework`) — 조사 결과 이것이 외부 URL을 여는 SDK 경로다. ⚠️ 미확인 2건: ⓐ 외부 브라우저로 열리는지 인앱 웹뷰인지 문서에 구분이 없다 ⓑ "허용된 경우에만 제한적으로" 조항 — 뉴스 원문 링크가 운영 정책상 허용 대상인지. 둘 다 샌드박스 실기기에서 확인하고, ⓑ는 심사 제출 전 운영 정책 문서(`policy` 계열 md)로 확정한다.
...
- **T1. 스캐폴딩** — `create-ait-app`(또는 콘솔 안내 절차)으로 Vite+React+TS+TDS 골격. 샌드박스에서 빈 화면 표시 + **내비게이션 바 뒤로가기 ↔ history 연동 실측**(4절 미확인 해소).
